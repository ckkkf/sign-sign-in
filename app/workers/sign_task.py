...
class SignTaskThread(QThread):
    ...
    @staticmethod
    def _service_provider(config: dict) -> str:
        return str((config.get("input") or config).get("serviceProvider", "xyb")).strip().lower() or "xyb"
    ...
    def wait_payload(self, proxy, source):
        last = time.time()
        channel = CodeChannel.instance()
        ...
        def heartbeat():
            ...
            if time.time() - last > 1.0:
                if get_system_proxy() != proxy:
                    logging.warning(f"system proxy changed to {get_system_proxy() or 'direct'}, reset to {proxy}")
                    set_proxy(proxy)
                last = time.time()

        return channel.wait_payload(timeout_seconds=120, source=source, stop_check=self.check_stop, heartbeat=heartbeat)
...
class GetCodeAndSessionThread(QThread):
    ...
    def run(self):
        try:
            ...
            ### 获取配置文件相关
            config = read_config(self.config_file)
            service_provider = SignTaskThread._service_provider(config)
            ...
            ### 唤起微信小程序
            weixin_urls = [
                f"weixin://launchapplet/?appid={LAISHIXI_APP_ID if service_provider == 'laishixi' else XYB_APP_ID}",
                f"weixin://launchapplet?appid={LAISHIXI_APP_ID if service_provider == 'laishixi' else XYB_APP_ID}",
            ]
            platform_name = "Laishixi" if service_provider == "laishixi" else "XYB"
            try:
                self.kill_wechat_before_launch()
                self.wake_applet_with_retry(weixin_urls)
                logging.info("wake command sent to WeChat")
                self.applet_wake_signal.emit()
            except Exception as e:
                # Continue when weixin URL scheme is unavailable.
            ...
            if service_provider == "laishixi":
                ...
                # get JSESSIONID
                logging.info("acquiring JSESSIONID...")
